...
func TestLocalE2EChargesEveryAvoidViolationThroughStakeEnforcer(t *testing.T) {
	...
	enforcer, err := newEnforcerWithBackend(ctx, "local", backend, stakeAddr, keyHex(enforcerKey), nil)
	if err != nil {
	...
	if got := readTokenBalance(t, token, burnAddress); got.Cmp(totalPenalty) != 0 {
...
func TestLocalE2EChargesEthereumAndPolygonConfiguredChains(t *testing.T) {
	...
	ethereumEnforcer, err := newEnforcerWithBackend(ctx, "ethereum", ethereum.backend, ethereum.stakeAddr, keyHex(enforcerKey), nil)
	if err != nil {
		t.Fatalf("new ethereum enforcer: %v", err)
	}
	polygonEnforcer, err := newEnforcerWithBackend(ctx, "polygon", polygon.backend, polygon.stakeAddr, keyHex(enforcerKey), nil)
	if err != nil {
		t.Fatalf("new polygon enforcer: %v", err)
	}
	enforcer := &Enforcer{
		privateKeyHex: keyHex(enforcerKey),
		chains: map[string]*chainClient{
			"ethereum": ethereumEnforcer.chains["ethereum"],
			"polygon":  polygonEnforcer.chains["polygon"],
		},
	}
...
func TestLocalE2ESchedulerChargesMissedDoGoalThroughStakeEnforcer(t *testing.T) {
	...
	enforcer, err := newEnforcerWithBackend(ctx, "ethereum", chain.backend, chain.stakeAddr, keyHex(enforcerKey), nil)
	if err != nil {
	...
	if got := readTokenBalance(t, chain.token, burnAddress); got.Cmp(chain.stakeAmount) != 0 {
