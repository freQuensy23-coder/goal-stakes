...
Usage:
  ...
  ENV_FILE=.env.mainnet.local scripts/live_mainnet_gate.sh burn
...
run_preflight() {
  ...
  echo "  ENV_FILE=$ENV_FILE scripts/live_mainnet_gate.sh burn"
}
...
const goal = await request("/api/v1/goals", {
  ...
  body: {
    ...
    description: "Created by scripts/live_mainnet_gate.sh burn through the public API.",
    type: "avoid",
    cadence: "daily",
    stake_amount: amount,
