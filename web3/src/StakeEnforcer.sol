...
/// @title StakeEnforcer
/// @notice Pulls a forfeited stake from a user's ERC20 allowance and burns it.
///
/// Design guarantees:
/// - IV1: forfeited funds can ONLY go to {BURN}. The destination is a compile-time
///   constant; there is no setter, no parameter, no code path that redirects it.
/// - IV2: the contract never takes custody. A penalty is a single
///   transferFrom(user -> BURN) bounded by the user's current allowance; no token
///   balance is held between calls.
/// - AS2: BURN is the unrecoverable dead address 0x..dEaD, NOT 0x0 (USDC reverts on 0x0).
contract StakeEnforcer {
    /// @notice The ONLY destination forfeited stake can ever reach (IV1, AS2).
    /// Hardcoded and immutable by construction — no function can change it.
    ...
    /// @notice Emitted when a stake is forfeited and burned.
    ...
    /// @notice Burn `amount` of `token` pulled from `user`'s allowance to this contract.
    /// @dev Single transferFrom(user -> BURN): the contract never holds the funds (IV2),
    /// and the destination is the hardcoded {BURN} constant only (IV1).
